...
class JOSC_com_community extends JOSC_component {
	/* specific properties of community if needed */
	private $id;
	private $component;
	private $componentTable = '#__users';
	private $componentTableFieldId = 'id';
	private $componentTableFieldTitle = 'name';
	private $componentTableFieldDescription = 'username';
	private $componentCategoryTable = '#__ReplaceTable';
	private $componentSectionTable = '#__ReplaceTable';
	private $feedName = 'jomsocialRSS';
	private $singleView = 'profile';
	...
	function linkToContent($contentId, $commentId='') {
		...
		$link = CRoute::_('index.php?option=com_community&view=profile&userid='.$contentId) . $comment;
		if($appl->isAdmin()) {
			$link = JURI::root().$link;
		}
