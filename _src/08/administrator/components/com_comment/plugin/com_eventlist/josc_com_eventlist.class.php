...
class JOSC_com_eventlist extends JOSC_component {
	private $feedName = 'eventlistRSS';
	private $component = 'com_eventlist';
	...
	function createFeed() {
		...
		$component = 'com_eventlist';

		$database = JFactory::getDBO();
		$database->setQuery("SELECT title, datdescription FROM #__eventlist_events WHERE id='$contentid'");
		...
		$rss->description = $content->datdescription;
		$rss->link = JURI :: base();
	...
	function linkToContent($contentId, $commentId='', $joscclean=false, $admin=false) {
		...
		$url = JRoute::_(JURI::root() . "index.php?option=com_eventlist&view=details&id=$contentId"
	...
	function getViewJoinQuery($alias, $contentid) {
		$leftjoin = "\n LEFT JOIN #__eventlist_events  AS $alias ON $alias.id = $contentid ";
		return $leftjoin;
	}
	...
	function mod_commentsGetMostCommentedQuery($secids, $catids, $maxlines) {
		...
		$query = "SELECT COUNT(c.id) AS countid, ct.id, ct.title "
				. "\n FROM `#__comment`    AS c "
				. "\n INNER JOIN `#__eventlist_events`  AS ct  ON ct.id = c.contentid "
	...
	function getCatSecAliasMod($contentid) {
		$database = & JFactory::getDBO();
		$query = 'SELECT  c.alias '
				. ' FROM ' . $database->nameQuote('#__eventlist_events') . 'AS c'
