...
class JOSC_com_hwdvideoshare extends JOSC_component {

	private $feedName = 'hwdvideoshareRSS';
	private $component = 'com_hwdvideoshare';
    ...
    function createFeed() {
		...
		$component = 'com_hwdvideoshare';

		$database = JFactory::getDBO();
		$database->setQuery("SELECT title, description FROM #__hwdvidsvideos WHERE id='$contentid'");
    ...
    public function checkVisual($contentId=0) {
		...
		return ($option == 'com_hwdvideoshare' && $task == 'viewvideo');
    }
    ...
    public function linkToContent($contentId, $commentId='', $joscclean=false, $admin=false) {
		...
		$url = JRoute::_('index.php?option=com_hwdvideoshare&task=viewvideo'.
					( $menuid ? "&Itemid=$menuid" : "" ) .'&video_id='.
    ...
    public function getViewJoinQuery($alias, $contentid) {
		$leftjoin	= "\n LEFT JOIN #__hwdvidsvideos  AS $alias ON $alias.id = $contentid ";
		return $leftjoin;
    }
    ...
    public function mod_commentsGetMostCommentedQuery($secids, $catids, $maxlines) {
		...
		$query 	=  	"SELECT COUNT(c.id) AS countid, ct.id, ct.title "
			.	"\n FROM `#__comment`    AS c "
			.  "\n INNER JOIN `#__hwdvidsvideos`    AS ct  ON ct.id = c.contentid "
			.  	"\n WHERE c.published='1' "
