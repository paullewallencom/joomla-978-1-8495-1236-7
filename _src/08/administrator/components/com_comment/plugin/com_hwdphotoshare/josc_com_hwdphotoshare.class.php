...
class JOSC_com_hwdphotoshare extends JOSC_component {

	private $component;
	private $componentTable ='#__hwdpsphotos';
	private $componentTableFieldTitle = 'title';
	private $componentTableFieldDescription = 'caption';
	private $feedName = 'com_hwdphotoshareFeed';
    ...
    public function checkVisual($contentId=0) {
		...
		return ($option == 'com_hwdphotoshare' && ($task == 'viewphoto' || $task == 'ajax_grabPhotoReload'));
    }
    ...
    public function linkToContent($contentId, $commentId='', $joscclean=false, $admin=false) {
		...
		$url = JRoute::_('index.php?option=com_hwdphotoshare&task=viewphoto'.
					( $menuid ? "&Itemid=$menuid" : "" ) .'&photo_id='.
	 ...
	 *  getItemid
	 */
    function getItemid( $component='com_hwdphotoshare') {
		static $ids;
		if( !isset($ids) ) {
			$ids = array();
		}
    ...
    public function getObjectIdOption($id=0, $select=true) {
		...
		$content = array();
		$query 	= "SELECT photo_id as id, title"
			. "\n FROM #__hwdpsphotos "
			. "  WHERE published=1"
			. ($id ? "\n AND id = $id":"")
		;
    ...
    public function getViewJoinQuery($alias, $contentid) {
		$leftjoin	= "\n LEFT JOIN #__hwdpsphotos  AS $alias ON $alias.id = $contentid ";
		return $leftjoin;
    }
    ...
    public function mod_commentsGetMostCommentedQuery($secids, $catids, $maxlines) {
		...
		if ($catids)
			 ...
			 */
		$query 	=  	"SELECT COUNT(c.id) AS countid, c.contentid, ct.id, "
			. ' CASE WHEN CHAR_LENGTH(ct.title) THEN ct.title ELSE CONCAT_WS(\':\',"photo_id", ct.id) END as title'
			.	"\n FROM `#__comment`    AS c "
			.  "\n INNER JOIN `#__hwdpsphotos`    AS ct  ON ct.id = c.contentid "
			.  	"\n WHERE c.published='1' "
