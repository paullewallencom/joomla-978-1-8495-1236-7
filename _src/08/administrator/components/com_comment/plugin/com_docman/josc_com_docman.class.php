...
class JOSC_com_docman extends JOSC_component {
	private $component = 'com_docman';
	private $feedName = 'docmanRSS';
	...
	function createFeed() {
		...
		$database->setQuery("SELECT * FROM #__docman WHERE id='$contentid'");
		...
		$rss->title = $content->dmname . ' - comments';
		$rss->description = $content->dmdescription;
		$rss->link = JURI :: base();
	...
	function linkToContent($contentId, $commentId='', $joscclean=false, $admin=false) {
		...
		$url = JURI::root() . "index.php?option=com_docman&task=doc_details&gid=$contentId" . ( $menuid ? "&Itemid=$menuid" : "" );
		$url .= $add;
